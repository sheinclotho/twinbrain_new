...
## [V5.50] 2026-03-02 — InfoNCE 梯度均衡修复：pred_r2 连续下降根因

### 问题

训练至 epoch 20-40 时，pred_r2 指标出现持续下降：
- pred_r2_fmri: 0.209（ep20）→ 0.205（ep25）→ 0.166（ep30）→ 0.159（ep35）→ 0.177（ep40）
- pred_r2_eeg: 0.156（ep20）→ 0.146（ep25）→ 0.165（ep30）→ 0.140（ep35）→ 0.146（ep40）

重建 R² 同期持续改善（r2_fmri: 0.983→0.984；r2_eeg: 0.736→0.789），验证损失亦下降，
说明模型整体在学习，但**预测能力的梯度被重建/InfoNCE 任务挤占**。

### 根因

InfoNCE（pred_nce）对比预测损失与 pred_sig 信号空间损失的**有效梯度比失衡**：

| 配置 | loss_scale | effective_nce | effective_pred_sig | 比值 |
|------|-----------|--------------|-------------------|------|
| V5.48（0.5, 2.0） | log(3230)/0.5 ≈ 16 | 2.0×16 = 32 | 6.0×1 = 6 | 5.4:1 |
| **V5.50（1.0, 1.0）** | **log(3230)/1.0 ≈ 8** | **1.0×8 = 8** | **6.0×1 = 6** | **1.3:1** |

本数据集（同步 EEG-fMRI，梯度噪声刺激范式）的 BOLD 滞后-1 自相关仅 ρ≈0.23，
远低于标准静息态 fMRI（ρ≈0.85-0.95）。在高 InfoNCE/pred_sig 比率（5.4:1）下，
predictor 在 warmup 结束后优先学习"区分不同预测窗口"（InfoNCE 目标），而非"准确预测
未来信号"（pred_sig 目标）——导致 pred_r2 随训练深入反而下降。

### 修复

```yaml
# configs/default.yaml
info_nce_temperature: 1.0   # 0.5 → 1.0：loss_scale 从 16 降至 8
task_priorities:
  pred_nce: 1.0             # 2.0 → 1.0：effective_nce 从 32 降至 8（比值 5.4:1 → 1.3:1）
```
...
| 文件 | 变更 |
|------|------|
| `configs/default.yaml` | `info_nce_temperature: 0.5 → 1.0`，`pred_nce: 2.0 → 1.0` |
| `AGENTS.md` | 记录 V5.50 修复 |
| `CHANGELOG.md` | 本条目 |
